...
(async()=>{
 ...
 if(!full){for(const [name,t] of [['bridge',.35],['victoria',1.0],['strike',1.69],['impact',2.08],['glass',2.28],['shards',2.48],['mark',2.94],['dock',3.92]]){const data=await page.evaluate(t=>window.film.frame(t),t);fs.writeFileSync(path.join(dir,`${mode}-${name}.jpg`),Buffer.from(data.split(',')[1],'base64'));}console.log('Stills ready');}
 else{
  const frames=path.join(dir,`frames-${mode}`);fs.mkdirSync(frames,{recursive:true});const fps=60,count=Math.round(3.95*fps),started=Date.now();
