...
window.makeKolkataFilm=async function(width=1080,height=1920){
 ...
 window.film={seek,duration:DURATION,canvas:renderer.domElement,width,height,frame(t,blur=false){if(blur&&t<3.71){seek(t-1/240);ax.globalAlpha=1;ax.drawImage(renderer.domElement,0,0);seek(t+1/240);ax.globalAlpha=.5;ax.drawImage(renderer.domElement,0,0);return accumulation.toDataURL('image/jpeg',.97);}seek(t);return renderer.domElement.toDataURL('image/jpeg',.97);},stats:()=>renderer.info.render};
