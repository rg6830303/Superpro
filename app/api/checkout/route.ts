...
export async function POST(req: Request) {
  ...
  try {
    ...
    // `isFree` covers a basket a code has wiped out: it is recorded as paid
    // rather than sent to a payment window. The floor below it is the same
    // rule from the gateway's side — Razorpay rejects anything under ₹1.
    const wantsOnline =
      !isFree && !wantsWallet && input.payment_method === "razorpay" && isRazorpayEnabled && totals.totalPaise >= 100;
